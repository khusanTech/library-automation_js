...
Feature: Book Categories
    ...
    Scenario Outline: Verify that all 21 books are displayed in exact orders for both admin and student users
        Given user is already on the login page
        And user is already logged in as "<user-type>"
        When user clicks Books link
        And user clicks the Book categories drop down
        Then book category name "<category>" should be included in the list of book categories
